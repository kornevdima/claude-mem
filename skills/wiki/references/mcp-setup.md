...
MCP lets a coding agent read and write vault notes directly without copy-paste. Four options ordered from simplest to most featureful.
...
You must do this in Obsidian (the REST API cannot enable itself from the agent alone):
...
Register this JSON with your **host product's MCP configuration** (exact UI or CLI differs — Cursor uses Settings → MCP; other tools may use a config file or their own CLI):
...
Name the server something stable (e.g. `obsidian-vault`) so tools can reference it.
...
Obsidian shipped a native CLI in v1.12 (2026). It exposes vault operations directly to the terminal. No REST API plugin, no MCP server, no self-signed certs, no TLS workarounds. Invoke it from the agent via your shell tool.
...
The `kepano/obsidian-skills` repo includes an `obsidian-cli` skill that wraps these commands as reusable patterns. Install it alongside this vault workflow for first-class CLI support.

---

## Claude Code convenience CLI

Claude Code can register either JSON block automatically and verify the connection from the terminal. Substitute the body of either Option A or Option B above for `<json>`.

```bash
# Register (use --scope user so the vault is available across all Claude Code projects)
claude mcp add-json obsidian-vault '<json>' --scope user
