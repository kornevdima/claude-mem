---
name: wiki-ingest-subagent
description: >
  INTERNAL: Task-only worker — invoked via the Agent/Task tool by the wiki-ingest skill,
...
- Update `wiki/index.md` or `wiki/log.md` (the caller / main thread does this after all subagents finish)
- Update `wiki/hot.md` (the caller does this at the end)
